...
const STORAGE_KEY = "pdf-split-cleaner-state-v1";
let pdfDoc = null;
...
async function loadPdf() {
  ...
  try {
    ...
    for (let pageNum = 1; pageNum <= pdfDoc.numPages; pageNum += 1) {
      ...
      setStatus(`Rendered page ${pageNum}/${pdfDoc.numPages}`);
    }
    buildPatternChips();
    scanPatterns();
    setStatus(`Ready. ${pdfDoc.numPages} page(s) loaded. Edit the right side, then export or print.`);
    saveAutosnapshot();
  } catch (error) {
    console.error(error);
    setStatus("Could not open that PDF.");
    alert("Could not open that PDF. It may be encrypted, damaged, or blocked by the browser.");
  }
}
...
async function renderPage(pageNum) {
  ...
  await addImagePlaceholders(page, viewport, editPage.querySelector(".page-content"), pageNum);
}
...
async function addImagePlaceholders(page, viewport, layer, pageNum) {
  try {
    const operatorList = await page.getOperatorList();
    const fns = operatorList.fnArray || [];
    let count = 0;
    fns.forEach((fn, index) => {
      const name = Object.entries(pdfjsLib.OPS).find(([, value]) => value === fn)?.[0] || "";
      if (!/paint.*Image|paintImage|paintJpeg|paintInlineImage/i.test(name)) return;
      count += 1;
      const placeholder = document.createElement("div");
      placeholder.className = "image-item";
      placeholder.dataset.page = String(pageNum);
      placeholder.dataset.index = `img-${index}`;
      placeholder.style.left = `${20 + (count % 3) * 90}px`;
      placeholder.style.top = `${20 + Math.floor(count / 3) * 70}px`;
      placeholder.style.width = "80px";
      placeholder.style.height = "55px";
      placeholder.addEventListener("click", event => { event.stopPropagation(); selectElement(placeholder); });
      layer.appendChild(placeholder);
    });
  } catch (error) {
    console.info("Image placeholder detection skipped", error);
  }
}
...
function jumpToElement(el) {
  ...
  if (syncScroll.checked) {
    const originalPage = originalViewer.querySelector(`.original-page[data-page="${el.dataset.page}"]`);
    originalPage?.scrollIntoView({ behavior: "smooth", block: "start" });
  }
}
...
function hideElements(elements) {
  pushUndo(elements);
  elements.forEach(el => { el.style.display = "none"; el.dataset.hidden = "true"; });
  scanPatterns();
}
...
function toggleSelectedCollapse() {
  ...
  if (selectedEl.tagName.toLowerCase() === "details") {
    selectedEl.open = !selectedEl.open;
    return;
  }
...
function deleteNearbyOptions() {
  ...
  if (!options.length) {
    alert("No nearby multiple-choice options found. Try selecting one option directly or use the MCQ group.");
    return;
  }
...
function insertCollapsibleBlock() {
  ...
  if (selectedEl && selectedEl.closest(".edit-page") === page) {
...
function collectEditState() {
  return {
    app: "pdf-split-cleaner",
    version: 1,
    fileName: currentFileName,
    exportedAt: new Date().toISOString(),
    html: editableViewer.innerHTML,
    plainMode: plainMode.checked,
    showDeleted: showDeleted.checked
  };
}

function downloadState() {
  download(`${safeName(currentFileName)}-edit-state.json`, JSON.stringify(collectEditState(), null, 2), "application/json");
}
...
function importEditState(event) {
  ...
  reader.onload = () => {
    try {
      ...
      rebindEditableEvents();
      buildPatternChips();
      scanPatterns();
    } catch (error) {
      alert("Could not import that edit state.");
    } finally {
      event.target.value = "";
    }
  };
...
function rebindEditableEvents() {
  ...
  editableViewer.querySelectorAll(".image-item,.inserted-block").forEach(el => {
    el.addEventListener("click", event => { event.stopPropagation(); selectElement(el); });
  });
}

function saveAutosnapshot() {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(collectEditState())); } catch {}
}
...
function safeName(value) {
  return String(value || "cleaned-pdf").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "cleaned-pdf";
}
